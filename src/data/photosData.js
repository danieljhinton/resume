/**
 * Photo gallery slots. Fill in `src` (e.g. '/assets/photos/whatever.jpg') and
 * flip `status` to 'ready' to have Photos.jsx render the real thumbnail —
 * no component changes needed.
 */

export const photos = [
  { id: 'photo-01', caption: 'Halo Head Office, Stowmarket, 2026', src: '/assets/photos/1.jpg', status: 'ready' },
  { id: 'photo-02', caption: 'MSP Masterclass, Auckland, 2026', src: '/assets/photos/2.jpg', status: 'ready' },
  { id: 'photo-03', caption: 'MSP Masterclass, Melbourne, 2026', src: '/assets/photos/3.jpg', status: 'ready' },
  { id: 'photo-04', caption: 'MSP Masterclass, Auckland, 2026', src: '/assets/photos/4.jpg', status: 'ready' },
  { id: 'photo-05', caption: 'MSP Masterclass, Melbourne, 2026', src: '/assets/photos/5.jpg', status: 'ready' },
  { id: 'photo-06', caption: 'MSP Masterclass, Melbourne, 2026', src: '/assets/photos/6.jpg', status: 'ready' },
  { id: 'photo-07', caption: 'MSP Masterclass, Melbourne, 2026', src: '/assets/photos/7.jpg', status: 'ready' },
  { id: 'photo-08', caption: 'MSP Masterclass, Melbourne, 2026', src: '/assets/photos/8.jpg', status: 'ready' },
  { id: 'photo-09', caption: 'MSP Masterclass, Melbourne, 2026', src: '/assets/photos/9.jpg', status: 'ready' },
  { id: 'photo-10', caption: 'MSP Masterclass, Auckland, 2026', src: '/assets/photos/10.jpg', status: 'ready' },
  { id: 'photo-11', caption: 'Kaseya Connect APAC Conference, Sydney, 2026', src: '/assets/photos/11.jpg', status: 'ready' },
  { id: 'photo-12', caption: 'MSP Masterclass, Auckland, 2026', src: '/assets/photos/12.jpg', status: 'ready' },
  { id: 'photo-13', caption: 'MSP Masterclass, Sydney, 2026', src: '/assets/photos/13.jpg', status: 'ready' },
  { id: 'photo-14', caption: 'Gartner Symposium, Gold Coast, 2026', src: '/assets/photos/14.jpg', status: 'ready' },
  { id: 'photo-15', caption: 'MSP Masterclass, Sydney, 2026', src: '/assets/photos/15.jpg', status: 'ready' },
  { id: 'photo-16', caption: 'Gainsight Pulse Conference, Las Vegas, 2026', src: '/assets/photos/16.jpg', status: 'ready' },
  { id: 'photo-17', caption: 'Halo U.S. West Coast Office, Bellingham, 2026', src: '/assets/photos/17.jpg', status: 'ready' },
  { id: 'photo-18', caption: 'Gartner Symposium, Gold Coast, 2026', src: '/assets/photos/18.jpg', status: 'ready' },
  { id: 'photo-19', caption: 'Professional Headshot, 2026', src: '/assets/photos/19.jpg', status: 'ready' },
];
